...
|------|------|
| `gui.py` | GUI 图形界面入口 |
| `cli.py` | CLI 命令行入口（交互模式 + Agent 非交互模式） |
| `process_language.py` | 核心质检引擎 |
| `AGENT_INSTRUCTIONS.md` | Agent 自动化操作指令 |
| `utils/` | 检测模块（变量/标签/术语/句式/UI/AI） |

## 📋 更新日志

### v3.2 — 2026-03-24
- 🌐 **多语言提示词** — AI 审查提示词根据选择的语言动态生成（中英/中印尼/中法/中德...）
- 🗂️ **结果归档** — 每次输出自动归档到 `output/{文件名}_{语言}_{时间戳}/`，根目录只保留最新结果
- 🆔 **列名兼容** — 支持 "映射ID"、"GameID" 等非标准 ID 列名
- 🇮🇩 **印尼语** — 新增 `idn` 语言选项

### v3.1 — 2026-03-19
- 🤖 **Agent 管道模式** — `--agent prepare/merge` 非交互模式，支持 openclaw 等 AI Agent 全自动操作
- 📋 **剪贴板 AI 审查** — GUI/CLI 引导式流程：复制提示词 → 粘贴到 ChatGPT → 粘贴结果
- ⬅️ **上一批撤回** — AI 审查支持返回上一批重做
- 📏 **批次大小可选** — 200 / 500 / 1000 行/批
- 📖 **术语注入提示词** — AI 审查自动附带当批涉及的标准术语表

### v3.0 — 2026-03-19
- 🔍 机审 + 🤖 AI审 两阶段质检流程
- GUI 图形界面 + CLI 命令行双入口
- 术语库支持 Excel 和 JSON 格式
- 输出：result.xlsx（2 sheets）+ report.xlsx（4 sheets）

### v1.0 — 2026-03-19
- 初始版本：变量检查、BBCode 标签、术语校验、句式一致性、中文残留、UI 识别

---

*创建时间: 2026-03-19 | 最后更新: 2026-03-24*
