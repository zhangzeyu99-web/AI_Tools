...
2. Columns: Time | Location | Issue Type (CN) | Original | Better {target_lang} | Analysis (CN)
3. Do NOT output Markdown code blocks.
"""

# --- 模式 1：标准模式 Prompt (支持续写) ---
STANDARD_INIT_PROMPT = """
[Task]
Audit the video from **00:00** as far as you can.
If the video is long, stop when you reach your output limit, I will ask you to continue.
...
Keep listing issues until the end of the video or output limit.
"""

# --- 模式 2：高密度模式 Prompt (30s 切片) ---
DENSITY_SEGMENT_PROMPT = """
[STRICT TIMEFRAME]
Audit ONLY from **{start_time}** to **{end_time}**.
(Do not look before {start_time} or after {end_time})

[Instruction]
1. This is a 30-second High-Density Scan.
2. List EVERY single issue (Spelling, Overlap, Consistency).
3. If no errors in this chunk, output nothing.
...
class VideoLocalizationApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("Gemini 3 LQA 双模终极版 (v19.0 Standard/Density)")
    ...
    def _init_video_ui(self, parent):
        ...
        ctk.CTkLabel(mode_frame, text="视频时长(分):").pack(side="left", padx=5)
        self.entry_duration = ctk.CTkEntry(mode_frame, width=50)
        self.entry_duration.insert(0, "5") # 默认5分钟
        self.entry_duration.pack(side="left", padx=5)

        ctk.CTkLabel(mode_frame, text="模式:").pack(side="left", padx=5)
        self.mode_var = tk.StringVar(value="standard")
        
        self.rb_standard = ctk.CTkRadioButton(mode_frame, text="基础模式 (长视频 <45m)", variable=self.mode_var, value="standard")
        self.rb_standard.pack(side="left", padx=5)
        
        self.rb_density = ctk.CTkRadioButton(mode_frame, text="高密度模式 (30s切片 <10m)", variable=self.mode_var, value="density")
        ...
        self.txt_video_out.insert("0.0", "👋 准备就绪。\n【基础模式】适合大多数情况，自动检测结尾并续写。\n【高密度模式】将强制每30秒交互一次，适合UI极多且容易被忽略的场景。")
    ...
    def parse_last_timestamp(self, text):
        # 查找文本中出现的最后一个时间戳 (格式 0:00, 12:30 等)
    ...
    def run_video_logic(self):
        ...
        try:
            total_minutes = int(self.entry_duration.get())
        except:
            total_minutes = 5
        ...
        try:
            ...
            if mode == "density":
                if total_minutes > 10: 
                    self.txt_video_out.insert("0.0", "⚠️ 警告：高密度模式建议用于10分钟以内视频。超时部分可能被忽略。\n")
                    total_minutes = 10
                ...
                for i in range(steps):
                    start_sec = i * 30
                    end_sec = (i + 1) * 30
                    start_str = self.seconds_to_hms(start_sec)
                    end_str = self.seconds_to_hms(end_sec)
                    ...
                    self.update_status(f"高密度扫描: {start_str} - {end_str} ({i+1}/{steps})...")
            ...
            # --- 分支 2: 基础模式 (Standard - Auto Continue) ---
            else:
                self.update_status("正在进行全时长审计 (初始轮)...")
                # 1. 初始请求
                ...
                # 只要最后检测到的时间 < 总时长 - 15秒缓冲，就继续追问
                retry_count = 0
                while last_ts_sec < (total_seconds - 15) and retry_count < 10:
                    retry_count += 1
                    last_ts_str = self.seconds_to_hms(last_ts_sec)
                    
                    self.update_status(f"发现未完成 (停在 {last_ts_str})，正在续写 (第{retry_count}轮)...")
